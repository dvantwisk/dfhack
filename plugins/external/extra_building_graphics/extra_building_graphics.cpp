...
DFHACK_PLUGIN("extra_building_graphics");
...
static const df::itemdef_handlerst itemdefs = df::global::world->raws.itemdefs;
static const df::descriptor_handlerst descriptors = df::global::world->raws.descriptors;
...
struct WellRecolorBufferInterpose : df::building_wellst {
    ...
    DEFINE_VMETHOD_INTERPOSE(void, drawBuilding, (uint32_t a, df::building_drawbuffer* buf, int16_t b))
    {
        ...
        int z = *df::global::window_z - Gui::getDepthAt(x1, y1);
...
struct FarmRecolorBufferInterpose : df::building_farmplotst {
    ...
    DEFINE_VMETHOD_INTERPOSE(void, drawBuilding, (uint32_t a, df::building_drawbuffer* buf, int16_t b))
    {
        ...
        if (it != swapped_texpos_cache_farmplot.end()) {
            ...
            for (int i = 0; i < 4; ++i) {
                if (plant_id[id] > -1) {
                    base_texture = set_pfarmplot_planted[0];
                }
            }
            df::palettest material_palette = df::global::world->raws.descriptors.colors[color]->palette;
            //int32_t base_texpos = buf->building_one_texpos[0][0];
...
struct PavedRoadRecolorBufferInterpose : df::building_road_pavedst {
    ...
    DEFINE_VMETHOD_INTERPOSE(void, drawBuilding, (uint32_t a, df::building_drawbuffer* buf, int16_t b))
    {
        ...
        if (it != swapped_texpos_cache_paved_road.end()) {
            ...
            df::palettest material_palette = descriptors.colors[color]->palette;
            int32_t base_texpos = buf->building_one_texpos[0][0];
            SDL_Surface* base_surface = getSurfaceByTexpos(base_texpos);
            TexposHandle base_texture = loadTexture(base_surface);
            swapped_texpos = swapPalettes(base_texture, base_inorganic_palette, material_palette);
        ...
        //std::cerr << "Got read texpos: " << getTexposByHandle(swapped_texpos) << "\n";
...
struct TractionBenchRecolorBufferInterpose : df::building_traction_benchst {
    ...
    DEFINE_VMETHOD_INTERPOSE(void, drawBuilding, (uint32_t a, df::building_drawbuffer* buf, int16_t b))
    {
        ...
        for (size_t i = 0; i < itemdefs.traction_bench_graphics_info.size(); ++i) {
            auto iflags = itemdefs.traction_bench_graphics_info[i]->flags;
            ...
            if (item_color == color + 1) {
                buf->building_one_texpos[0][0] = itemdefs.traction_bench_graphics_info[i]->texpos;
                break;
            }
        }
    }
...
// May need to check for water level flag on building draw_buffers to check if to reorganize layers.
struct BuildingWaterInterpose : public df::renderer_2d {
    ...
    DEFINE_VMETHOD_INTERPOSE(void, update_viewport_tile,
        ...
        if (false) { //vp->screentexpos_building_one[x * vp->dim_y + y]) {
            std::cerr << "At Pos x: " << x + *df::global::window_x << " y: " << y + *df::global::window_y << "\n";
            std::cerr << "Background: " << vp->screentexpos_background[x * vp->dim_y + y] << "\n";
            std::cerr << "Background two: " << vp->screentexpos_background_two[x * vp->dim_y + y] << "\n";
            std::cerr << "Liquid: " << liquid_val << "\n";
        }
...
command_result extra_building_graphics_cmd(color_ostream &out, std::vector<std::string> & args)
{
    bool enable = true;
    ...
    if (enable) {
        ...
        df::tile_pagest* farmplot_not_planted_tilepage = getTilePage("ALT_FLOORS");
        ...
        if (!INTERPOSE_HOOK(TractionBenchRecolorBufferInterpose, drawBuilding).apply(true)) {
            out.printerr("Failed to interpose draw_building.\n");
            return CR_FAILURE;
        }
        std::cerr << "5\n";
        ...
        std::cerr << "In enabled hooks before roads\n";
        ...
        std::cerr << "In enabled hooks before well\n";
        ...
        out.print("Interpose on draw_building enabled.\n");
    } else {
...
commands.push_back(PluginCommand(
    "extra_building_graphics",
    "Intercept building rendering",
    extra_building_graphics_cmd
));
